...
const fetchCurrentUserInternRanking = async (companyCode: string): Promise<void> => {
  try {
    const response = await axios.get('http://10.61.11.243:3010/internRanking', { headers: { 'company_code': companyCode } });
    if (response.data)
...
const fetchCurrentExternCompanyRanking = async (): Promise<void> => {
  try {
    const response = await axios.get('http://10.61.11.243:3010/externRanking');
    if (response.data)
